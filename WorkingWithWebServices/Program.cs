...
/* Needed to add additional references:
 * System.Web.Extensions (for System.Web.Script namespace)
 * System.Runtime.Serialization (for System.Runtime.Serialization and System.Runtime.Serialization.JSON)
 * // System.Runtime.Remoting (for async class)
 */
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            Action action = async () =>
            {
                ...
                {
                    // basic implementation of REST and JSON using a web client, which is a wrapper around the HTTP client and it exists because it handles common operations

                    // fetch data (as JSON string)
                    var url = new Uri("http://localhost:1234/MyService.svc/json/4");
                    var client = new System.Net.WebClient();
                    //var json = await client.DownloadStringTaskAsync(url); // System.Net.WebClient does not contain a definition for 'DownloadStringTaskAsync' and not extension method 'DownloadStringTaskAsync' accepting a first argument type of 'System.New.WebClient' could be found
                    var json = await client.DownloadStringAsync(url);
